(function () {
  ...
  const checkIfEmailAlreadyExists = async function (email) {
    // full URL to Spring Boot‑server
    const url = `${BASE}/exists?email=${encodeURIComponent(email)}`;

    const response = await fetch(url, { method: 'GET' });
    if (!response.ok) {
      throw new Error(`Network error: ${response.status}`);
    }
    return response.json();  // { presentInDatabase: boolean, msg: string }
  };
  ...
  /**
 * Check the email existence when the input loses focus.
 * It first validates format, then calls the backend, then updates UI.
 * @return {Promise<boolean>} true if both format and existence are valid.
 */
  const checkEmailExistence = async function () {
    ...
    try {
      const data = await checkIfEmailAlreadyExists(value);
      // data = { presentInDatabase: boolean, msg: string }
      ...
      validation.innerText = 'Ошибка связи с сервером';
      invalidate(validation, emailField);
      console.error(err);
      return false;
    }
  ...
  const signup = async function (e) {
    e.preventDefault();
      ...
      // Collecting pls from badge‑elements
    const languages = getPickedLanguages().map(li => li.innerText.trim().toLowerCase());

    const payload = {
      email: emailField.value.trim(),
      interestsPl: languages.join(',')
    };
    ...
    try {
      ...
      alert(
        `Success! Email "${created.email}" saved with id=${created.id}.\n` +
